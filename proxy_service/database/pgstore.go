...
func (s *PGStore) GetRoleByName(name string) (*Role, error) {
    var r Role
    if err := s.db.Where("name = ?", name).First(&r).Error; err != nil { return nil, err }
    return &r, nil
}
...
func (s *PGStore) GetAccountingRuleByID(id uint) (*AccountingRule, error) {
    ...
    if err := s.db.First(&a, id).Error; err != nil { return nil, err }
    return &a, nil
}
func (s *PGStore) GetAccountingRuleByEndpoint(endpoint string) (*AccountingRule, error) {
    var a AccountingRule
    if err := s.db.Where("endpoint = ?", endpoint).First(&a).Error; err != nil { return nil, err }
    return &a, nil
}
func (s *PGStore) UpdateAccountingRule(a *AccountingRule) error { return s.db.Save(a).Error }
func (s *PGStore) DeleteAccountingRule(id uint) error { return s.db.Delete(&AccountingRule{}, id).Error }
...
func (s *PGStore) GetCustomEndpointByPath(path string) (*CustomEndpoint, error) {
    var c CustomEndpoint
    if err := s.db.Where("path = ?", path).First(&c).Error; err != nil { return nil, err }
    return &c, nil
}
func (s *PGStore) UpdateCustomEndpoint(c *CustomEndpoint) error { return s.db.Save(c).Error }
func (s *PGStore) DeleteCustomEndpoint(id uint) error { return s.db.Delete(&CustomEndpoint{}, id).Error }
